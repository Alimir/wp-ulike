...
jQuery(window).load(function() {
    $('.wp_ulike_btn').each(function($index, $element) {
        var type = $($element).data('ulike-type');
        var status = $($element).data('ulike-status');
        var id = $($element).data('ulike-id');
        var uclass = $($element).data('ulike-class');
        var p_class = $($element).parent();
        jQuery.ajax({
            type: 'POST',
            cache: false,
            url: ulike_obj.ajaxurl,
            ...
            success: function(data) {
                p_class.html(jQuery(data).find('response_data').text());
            }
        });
    });
});