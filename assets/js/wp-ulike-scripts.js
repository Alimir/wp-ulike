		...
		if (id != '') {
			...
			jQuery.ajax({
			  ...
			  success: function(data) {
				if(status == 1){
				  p_class.html("<a data-ulike-id='"+id+"' data-ulike-type='"+type+"' data-ulike-status='2' class='wp_ulike_btn text'>" + ulike_obj.text_after_like + "</a><span class='count-box'>"+data+"</span>");
				}
				if(status == 2){
				  if(ulike_obj.return_initial_after_unlike == 1){
					...
					p_class.html("<a data-ulike-id='"+id+"' data-ulike-type='"+type+"' data-ulike-status='"+status+"' class='wp_ulike_btn text'>" + ulike_obj.text_after_unlike + "</a><span class='count-box'>"+data+"</span>");
				  }
				}
				if(status == 3){
