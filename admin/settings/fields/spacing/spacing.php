  ...
  class ULF_Field_spacing extends ULF_Fields {
    ...
    public function render() {
      ...
      if ( ! empty( $args['all'] ) ) {
        ...
        echo ( ! empty( $args['all_icon'] ) ) ? '<span class="ulf--label ulf--icon">'. $args['all_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[all]' ) ) .'" value="'. esc_attr( $value['all'] ) .'"'. $placeholder .' class="ulf-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        ...
        foreach ( $properties as $property ) {
          ...
          echo '<div class="ulf--input">';
          echo ( ! empty( $args[$property.'_icon'] ) ) ? '<span class="ulf--label ulf--icon">'. $args[$property.'_icon'] .'</span>' : '';
          echo '<input type="number" name="'. esc_attr( $this->field_name( '['. $property .']' ) ) .'" value="'. esc_attr( $value[$property] ) .'"'. $placeholder .' class="ulf-input-number'. esc_attr( $is_unit ) .'" step="any" />';
          echo ( $unit ) ? '<span class="ulf--label ulf--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
          echo '</div>';
